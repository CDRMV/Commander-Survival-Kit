local GetM28AIPath = function() for i, mod in __active_mods do if mod.uid == "fnewm028-v206-55b4-92b6-64398e7ge43f" then return mod.location end end end
local M28AIPath = GetM28AIPath()
...
function UpgradeUnit(oUnitToUpgrade, bUpdateUpgradeTracker)
        ...
        function()
            WaitSeconds(Tech2)
			DoUpgrades(oUnitToUpgrade, bUpdateUpgradeTracker)
			WaitSeconds(Tech3)
			DoUpgrades(oUnitToUpgrade, bUpdateUpgradeTracker)
			WaitSeconds(Elite)
			DoUpgrades(oUnitToUpgrade, bUpdateUpgradeTracker)
...
function DoUpgrades(oUnitToUpgrade, bUpdateUpgradeTracker)
    ...
    if oUnitJustBuilt:GetFractionComplete() == 1 and EntityCategoryContains(M28UnitInfo.refCategoryFactory + M28UnitInfo.refCategoryQuantumGateway, oUnitJustBuilt.UnitId) then
        ...
        if EntityCategoryContains(M28UnitInfo.refCategoryLandFactory, oUnitJustBuilt.UnitId) then
            ...
            if iPlateau and iLandZone then
                ...
                if not(M28Utilities.bLoudModActive or M28Utilities.bQuietModActive) and M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.reftiCoreZonesByPlateau][iPlateau]) == false then
                ...
                if not(bHaveStartInSameIslandOrLOUD) then
                    ...
                    if not(bHaveOtherFactoriesOfSameTech) then
                        ...
                        if (tFactoryLZData[M28Map.subrefLZMexCount] or 0) >= 3 or (tFactoryLZData[M28Map.subrefLZIslandRef] and ((M28Map.tAllPlateaus[iPlateau][M28Map.subrefPlateauIslandMexCount][tFactoryLZData[M28Map.subrefLZIslandRef]] or 0) >= 7 or ((M28Map.tAllPlateaus[iPlateau][M28Map.subrefPlateauIslandMexCount][tFactoryLZData[M28Map.subrefLZIslandRef]] or 0) >= 5 and tFactoryLZTeamData[M28Map.refiModDistancePercent] >= 0.2))) or (M28Utilities.bLoudModActive or M28Utilities.bQuietModActive) and (tFactoryLZData[M28Map.subrefLZMexCount] or 0) >= 2 then
    ...
    if M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.subreftoFriendlyActiveM28Brains]) == false then
        ...
        if bStallingMass then
            if not(bImminentThreat) then
                ...
                tCategoryAndEngineerTables[2] = { { M28Engineer.refActionBuildQuantumOptics, M28Engineer.refActionBuildHive, M28Engineer.refActionBuildT3Radar, M28Engineer.refActionBuildGameEnder, M28Engineer.refActionBuildLandExperimental, M28Engineer.refActionBuildSecondExperimental, M28Engineer.refActionNavalSpareAction, M28Engineer.refActionBuildT2Sonar, M28Engineer.refActionBuildThirdPower, M28Engineer.refActionBuildSecondAirFactory, M28Engineer.refActionBuildSecondLandFactory, M28Engineer.refActionBuildLandFactory, M28Engineer.refActionSAMCreep, M28Engineer.refActionBuildNavalFactory, M28Engineer.refActionAssistNavalFactory, M28Engineer.refActionBuildAirFactory, M28Engineer.refActionBuildT1Sonar, M28Engineer.refActionBuildT2Radar, M28Engineer.refActionBuildT1Radar, M28Engineer.refActionBuildQuantumGateway, M28Engineer.refActionBuildSecondPower, M28Engineer.refActionBuildEnergyStorage, M28Engineer.refActionBuildAirStaging, M28Engineer.refActionManageGameEnderTemplate, M28Engineer.refActionBuildShield, M28Engineer.refActionBuildSecondShield, M28Engineer.refActionBuildExperimental, M28Engineer.refActionAssistAirFactory, M28Engineer.refActionUpgradeBuilding, M28Engineer.refActionBuildPower },
                                                  { M28Engineer.refActionAssistUpgrade, M28Engineer.refActionBuildT3MassFab, M28Engineer.refActionBuildSecondMassStorage, M28Engineer.refActionBuildMassStorage, M28Engineer.refActionAssistMexUpgrade, M28Engineer.refActionSpare },
                                                  { M28Engineer.refActionBuildTML, M28Engineer.refActionFortifyFirebase, M28Engineer.refActionBuildSMD, M28Engineer.refActionBuildEmergencyArti }}
            end
        else
            --Power stall
            if not(bImminentThreat) then
            ...
            if bChangeRequired then
                ...
                if bPauseNotUnpause then
                    if M28Team.tTeamData[iTeam][M28Team.refbNeedResourcesForMissile] then iMassPerTickSavingNeeded = math.max(1, -M28Team.tTeamData[iTeam][M28Team.subrefiTeamNetMass])
                        ...
                        if -M28Team.tTeamData[iTeam][M28Team.subrefiTeamNetMass] > M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossMass] * 0.8 then --almost 80% stall rate
                            iMassPerTickSavingNeeded = math.max(1, -M28Team.tTeamData[iTeam][M28Team.subrefiTeamNetMass])
                        else
                                ...
                                if M28Conditions.IsTableOfUnitsStillValid(M28Team.tTeamData[iTeam][M28Team.subreftTeamUpgradingMexes]) then
                                    ...
                                    if M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossMass] <= 2.5 * M28Team.tTeamData[oBrain.M28Team][M28Team.subrefiActiveM28BrainCount] then
                                        ...
                                        for iMex, oMex in M28Team.tTeamData[iTeam][M28Team.subreftTeamUpgradingMexes] do
                                            if EntityCategoryContains(categories.TECH2 + categories.TECH3, oMex.UnitId) then
                                                bUpgradingT2OrT3Mex = true
                                                break
                                            end
                                        end
                                    ...
                                    if iMexesToPause > 0 and (M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossMass] >= 14 or (M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossMass] > 10 and M28Conditions.GetHighestOtherTeamT3MexCount(iTeam) > M28Team.tTeamData[iTeam][M28Team.refiMexCountByTech][3])) then
                                        local iMexPerPlayer
                                        if (M28Utilities.bLoudModActive or M28Utilities.bQuietModActive) or M28Map.iMapSize > 1024 then iMexPerPlayer = 1 + (oBrain[refiGrossMassBaseIncome] / 6)*0.45
                                    ...
                                    if iMexesToPause > 0 and (M28Utilities.bLoudModActive or M28Utilities.bQuietModActive) and not(M28Team.tTeamData[iTeam][M28Team.refbPrioritiseProduction]) and oBrain[refiGrossMassBaseIncome] >= 3 then
                                    ...
                                    if (M28Utilities.bLoudModActive or M28Utilities.bQuietModActive) and M28Utilities.IsTableEmpty(tRelevantUnits) == false then
...
function ConsiderFutureMexUpgrade(oMex, iOverrideSecondsToWait)
    ...
    if not(iTimeToWait) then
        if iMexTechLevel == 1 then
            ...
            if M28UnitInfo.GetUnitLifetimeCount(oMex) == 1 and tLZOrWZTeamData[M28Map.refbBaseInSafePosition] and tLZOrWZData[M28Map.subrefLZMexCount] >= 6 and oMex:GetAIBrain()[refiBrainBuildRateMultiplier] <= 1 and M28Utilities.bFAFActive then
            ...
            elseif M28Map.iMapSize >= 1024 and tLZOrWZTeamData[M28Map.subrefLZbCoreBase] then
        ...
        elseif bT3MexCanBeUpgraded and (M28Utilities.bLoudModActive or M28Utilities.bQuietModActive) and iMexTechLevel == 3 then
            ...
            if aiBrain[refiGrossMassBaseIncome] < 25 and (not(M28Utilities.bLoudModActive or M28Utilities.bQuietModActive) or iMexesOnMap > 20 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount]) then
                iTimeToWait = 4 * 60 + 5 * 60 * (25-aiBrain[refiGrossMassBaseIncome]) / 10
            elseif (M28Utilities.bLoudModActive or M28Utilities.bQuietModActive) and aiBrain[refiGrossMassBaseIncome] < 12 and iMexesOnMap <= 20 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] then
                iTimeToWait = 2 * 60 + 2 * 60 * (25-aiBrain[refiGrossMassBaseIncome]) / 10
            else
                iTimeToWait = 4 * 60
            end
            if iMexesInZone < 4 and aiBrain[refiGrossMassBaseIncome] <= 40 and iMexesOnMap > 60 then
                iTimeToWait = iTimeToWait + 2 * 60 * (4 - (tLZData[M28Map.subrefLZMexCount] or 0))
            elseif iMexesInZone < 4 and aiBrain[refiGrossMassBaseIncome] <= 20 and iMexesOnMap <= 60 then
                iTimeToWait = iTimeToWait + 1 * 60 * (4 - (tLZData[M28Map.subrefLZMexCount] or 0))
            end
        end
        if (M28Utilities.bLoudModActive or M28Utilities.bQuietModActive) and M28UnitInfo.GetUnitLifetimeCount(oMex) == 1 and iMexTechLevel == 1 then
            if iMexesOnMap > 20 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] then
        ...
        if not(M28Team.tTeamData[iTeam][M28Team.refbFocusOnT1Spam]) then
            ...
            if M28Conditions.SafeToUpgradeUnit(oMex) and not(M28Team.tLandSubteamData[aiBrain.M28LandSubteam][M28Team.refbPrioritiseProduction]) then
                ...
                if not(M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingEnergy]) and (tLZOrWZTeamData[M28Map.subrefiActiveMexUpgrades] or 0) == 0 then
                    ...
                    if M28Utilities.bLoudModActive or M28Utilities.bQuietModActive then iTimeThreshold = 600
                    else
                        ...
                        else iTimeThreshold = 900
                        end
                    end
                    if iMexTechLevel == 2 then iTimeThreshold = iTimeThreshold * 1.5 end
                    if not(aiBrain[refiBrainResourceMultiplier] == 1) then
                        iTimeThreshold = iTimeThreshold * (1 + (aiBrain[refiBrainResourceMultiplier] - 1) * 0.5)
                    end
                    local iTeamUpgradingMexCount = 0
                    if M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.subreftTeamUpgradingMexes]) == false then
                    ...
                    local iMexesOfHigherTech = 0
                    local iUpgradingMexCount = 0
                    if M28Utilities.IsTableEmpty(M28Team.tTeamData[iTeam][M28Team.subreftTeamUpgradingMexes]) == false then
                        iUpgradingMexCount = table.getn(M28Team.tTeamData[iTeam][M28Team.subreftTeamUpgradingMexes])
                    end
                    if iMexTechLevel == 1 then
                        iMexesOfHigherTech = M28Team.tTeamData[iTeam][M28Team.refiUpgradedMexCount] / M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount]
                    else
                        iMexesOfHigherTech = aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryMex - categories.TECH1 - categories.TECH2)
                    end
                    if iMexesOfHigherTech <= 3 * (1 + iUpgradingMexCount) then  --e.g. want 3 t2 mex for every 1 t1 mex that are upgrading, so significantly increase the time to wait by up to 3 times
                    ...
                    if GetGameTimeSeconds() - (oMex[M28UnitInfo.refiTimeMexConstructed] or oMex[M28UnitInfo.refiTimeCreated]) >= iTimeThreshold and iUpgradingMexCount < M28Team.tTeamData[iTeam][M28Team.refiUpgradedMexCount] * 0.3 then
                        ...
                        local iMexesChecked = (tLZOrWZData[M28Map.subrefLZMexCount] or 0) --WZ uses same ref
                        if iMexesChecked < 4 then
                            local iMexesUpgrading = 0
                            if iPlateauOrZero > 0 then
                                if M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefAdjacentLandZones]) == false then
                                    for _, iAdjLZ in tLZOrWZData[M28Map.subrefAdjacentLandZones] do
                                        local tAdjLZData = M28Map.tAllPlateaus[iPlateauOrZero][M28Map.subrefPlateauLandZones][iAdjLZ]
                                        local tAdjLZTeamData = tAdjLZData[M28Map.subrefLZTeamData][iTeam]
                                        iMexesUpgrading = iMexesUpgrading + (tAdjLZTeamData[M28Map.subrefiActiveMexUpgrades] or 0)
                                        iMexesChecked = iMexesChecked + tAdjLZData[M28Map.subrefLZMexCount]
                                    end
                                end
                            else
                                if M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefAdjacentWaterZones]) == false then
                                    for _, iAdjWZ in tLZOrWZData[M28Map.subrefAdjacentWaterZones] do
                                        ...
                                        iMexesUpgrading = iMexesUpgrading + (tAdjWZTeamData[M28Map.subrefiActiveMexUpgrades] or 0)
                                        iMexesChecked = iMexesChecked + tAdjWZData[M28Map.subrefWZMexCount]
                                    end
                                end
                            end
                            if not(iMexesUpgrading == 0 or iMexesUpgrading < math.min(3, iMexesChecked / 8)) then
                ...
                if not(M28Conditions.ZoneWantsT1Spam(tLZOrWZTeamData, iTeam)) and (bUpgradeDueToHowLongHadMex or not(M28Conditions.TeamHasLowMass(iTeam)) or iMexTechLevel == 3 or ((tLZOrWZData[M28Map.subrefLZMexCount] or 0) >= 3 and (tLZOrWZTeamData[M28Map.subrefiActiveMexUpgrades] or 0) == 0 and (tLZOrWZTeamData[M28Map.subrefMexCountByTech][1] > 0 or aiBrain[refiGrossMassBaseIncome] >= 8 or (M28Utilities.bLoudModActive or M28Utilities.bQuietModActive)))) then
                    ...
                    if iMexTechLevel == 3 or
                            ...
                            tLZOrWZData[M28Map.subrefLZMexCount] >= 3 or
                            (iPlateauOrZero > 0 and tLZOrWZData[M28Map.subrefLZMexCount] >= math.max(2, M28Map.tAllPlateaus[iPlateauOrZero][M28Map.subrefPlateauTotalMexCount] * 0.5)) or
                        ...
                        if (iMexTechLevel ==3 and (M28Utilities.bLoudModActive or M28Utilities.bQuietModActive)) or (M28Conditions.TeamHasLowMass(iTeam) or (tLZOrWZTeamData[M28Map.subrefiActiveMexUpgrades] or 0) == 0 and not(M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingEnergy]) and
                                (not(M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingMass]) or
                                        ...
                                        (tLZOrWZData[M28Map.subrefLZMexCount] >= 3 and table.getn(M28Team.tTeamData[iTeam][M28Team.subreftTeamUpgradingMexes]) < M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] and aiBrain[refiGrossMassBaseIncome] >= 3.5 * iMexTechLevel and (iMexTechLevel == 1 or  table.getn(M28Team.tTeamData[iTeam][M28Team.subreftTeamUpgradingMexes]) < M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] * 0.5)) or
                    ...
                    ForkThread(ConsiderFutureMexUpgrade, oMex, 40)
                end
                ...
                if M28Team.tLandSubteamData[aiBrain.M28LandSubteam][M28Team.refbPrioritiseProduction] and M28Conditions.TeamHasLowMass(iTeam) and (not(tLZOrWZTeamData[M28Map.refbBaseInSafePosition]) or tLZOrWZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ] or (tLZOrWZTeamData[M28Map.subrefMexCountByTech][iMexTechLevel - 1] or 0) == 0) then
                    ...
                    if iMexTechLevel >= 3 or
                            ((tLZOrWZData[M28Map.subrefLZMexCount] > 1 or tLZOrWZTeamData[M28Map.subrefLZbCoreBase]) and
                                    ((M28Map.iMapSize >= 1024 and tLZOrWZTeamData[M28Map.subrefiActiveMexUpgrades] < 2) or tLZOrWZTeamData[M28Map.subrefiActiveMexUpgrades] < tLZOrWZData[M28Map.subrefLZMexCount] * 0.3 or ((M28Utilities.bLoudModActive or M28Utilities.bQuietModActive) and (EntityCategoryContains(M28UnitInfo.refCategoryT3Mex, oJustBuilt.UnitId) or (not(M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingMass]) and tLZOrWZTeamData[M28Map.subrefMexCountByTech][1] > 0 and iMexesOnMap <= 20 * M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount])))
                                            and (tLZOrWZTeamData[M28Map.subrefiActiveMexUpgrades] < math.max(1, tLZOrWZTeamData[M28Map.subrefMexCountByTech][2] * 0.5 + tLZOrWZData[M28Map.subrefLZMexCount] * 0.15 + tLZOrWZTeamData[M28Map.subrefMexCountByTech][3]) or (tLZOrWZTeamData[M28Map.subrefiActiveMexUpgrades] <= 1 and M28Map.iMapSize >= 1024) or ((M28Utilities.bLoudModActive or M28Utilities.bQuietModActive) and EntityCategoryContains(M28UnitInfo.refCategoryT3Mex, oJustBuilt.UnitId))))) then
                        ...
                        if tLZOrWZTeamData[M28Map.subrefMexCountByTech][1] > 0 or (iMexTechLevel >= 3 and (M28Utilities.bLoudModActive or M28Utilities.bQuietModActive or tLZOrWZTeamData[M28Map.subrefMexCountByTech][2] > 0)) or ((M28Utilities.bLoudModActive or M28Utilities.bQuietModActive) and tLZOrWZTeamData[M28Map.subrefiActiveMexUpgrades] == 0 and tLZOrWZData[M28Map.subrefLZMexCount] >= 3 and aiBrain[refiGrossMassBaseIncome] >= 15)  then --In LOUD, T3 mex upgrades are more efficient than t2 to t3 apparently
                                ...
                                elseif M28Utilities.IsTableEmpty(tLZOrWZTeamData[M28Map.subreftoLZOrWZAlliedUnits]) == false then --redundancy
                                    ...
                                    if M28Utilities.IsTableEmpty(tMexOfCategory) then
                                        ...
                                        if (tLZOrWZTeamData[M28Map.subrefiActiveMexUpgrades] or 0) == 0 then
                                            local iModDistThreshold = math.max(0.35, tLZOrWZTeamData[M28Map.refiModDistancePercent])
                                            ...
                                            if iPlateauOrZero then
                                                if M28Utilities.IsTableEmpty(tLZOrWZData[M28Map.subrefLZAdjacentLandZones]) == false then --for now will only to land zones not water zones since greater risk with water zones
                                                    for _, iAdjLZ in tLZOrWZData[M28Map.subrefLZAdjacentLandZones] do
                                                        ...
                                                        if tAdjLZTeamData[M28Map.refiModDistancePercent] <= iModDistThreshold and tAdjLZTeamData[M28Map.subrefTThreatEnemyCombatTotal] == 0 and tAdjLZTeamData[M28Map.refiEnemyAirToGroundThreat] == 0 and M28Utilities.IsTableEmpty(tAdjLZTeamData[M28Map.subreftoLZOrWZAlliedUnits]) == false then
