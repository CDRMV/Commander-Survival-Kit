...
function LayoutResourceGroup(group, groupType)
    ...
    LayoutHelpers.AtRightIn(group.income, group, 10)
    LayoutHelpers.AtTopIn(group.income, group)
    group.income:SetColor('ffb7e75f')
    
    LayoutHelpers.AtRightIn(group.expense, group, 10)
...
function TogglePanelAnimation(state)
    ...
    if UIUtil.GetAnimationPrefs() then
        if state or GUI.bg:IsHidden() then
			...
			import('/mods/Commander Survival Kit Timeos/UI/MainPanel.lua').ToggleMFDPanel(true)
			...
			import('/mods/Commander Survival Kit Timeos/UI/MainPanel.lua').ToggleMFDPanel(false)
