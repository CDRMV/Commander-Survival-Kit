...
MassFabPanel = ClassUI(Group) {
    ...
    __init = function(self, parent)
        ...
        LayoutHelpers.AtRightTopIn(self, parent, pos.right, 4)
    end,
    ...
    HandleEvent = function(self, event)
        if event.Type == "ButtonPress" and event.Modifiers.Middle then
            ...
            drag.OnMove = function(dragself, x, y)
                self.Right:Set(math.min(math.max(x - offX, self._parent.Left()), self._parent.Right() - self.Width()))
    ...
    _SavePosition = function(self)
        Prefs.SetToCurrentProfile("MassFabsPanelPos", {
            right = LayoutHelpers.InvScaleNumber(self.Right())
        })
    end,
