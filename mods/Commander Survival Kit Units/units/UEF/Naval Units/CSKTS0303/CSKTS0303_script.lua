...
CSKTS0303 = Class(TSeaUnit) {
    ...
    Weapons = {
		...
		PhalanxGun01 = Class(TAMPhalanxWeapon) {
            PlayFxWeaponUnpackSequence = function(self)
                if not self.SpinManip then 
                    self.SpinManip = CreateRotator(self.unit, 'TMD1_Barrel', 'z', nil, 270, 180, 60)
		...
		PhalanxGun03 = Class(TAMPhalanxWeapon) {
            PlayFxWeaponUnpackSequence = function(self)
                if not self.SpinManip then 
                    self.SpinManip = CreateRotator(self.unit, 'TMD3_Barrel', 'z', nil, 270, 180, 60)
                    self.SpinManip:SetPrecedence(10)
                    self.unit.Trash:Add(self.SpinManip)
                end
                if self.SpinManip then
		...
		PhalanxGun04 = Class(TAMPhalanxWeapon) {
            PlayFxWeaponUnpackSequence = function(self)
                if not self.SpinManip then 
                    self.SpinManip = CreateRotator(self.unit, 'TMD4_Barrel', 'z', nil, 270, 180, 60)
                    self.SpinManip:SetPrecedence(10)
                    self.unit.Trash:Add(self.SpinManip)
                end
                if self.SpinManip then
    ...
    BuildAttachBone = 'Build01',
	BuildHoverAttachBone = 'Build02',
    ...
    OnScriptBitSet = function(self, bit)
        ...
        if bit == 1 then 
		self:RemoveBuildRestriction(categories.AIR)
		self:AddBuildRestriction(categories.HOVER)
        end
    end,
    ...
    OnScriptBitClear = function(self, bit)
		...
		self:RemoveBuildRestriction(categories.HOVER)
        end
    end,
