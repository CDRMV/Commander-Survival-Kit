...
TacNukeEffectController01 = Class(NullShell) {
    ...
    EffectThread = function(self)
        ...
        self:CreateProjectile('/mods/Reinforcement Manager/effects/Entities/Seraphim/TacNukeEffect01/TacNukeEffect01_proj.bp',0,FireballDomeYOffset,0,0,0,1)
        
        # Create projectile that controls plume effects
        local PlumeEffectYOffset = -0.25
        self:CreateProjectile('/mods/Reinforcement Manager/effects/Entities/Seraphim/TacNukeEffect02/TacNukeEffect02_proj.bp',0,PlumeEffectYOffset,0,0,0,1)        
        ...
        CreateDecal(position, orientation, 'Scorch_012_albedo', '', 'Albedo', 8, 8, 1200, 0, army) 
        CreateDecal(position, orientation, 'Crater01_normals', '', 'Normals', 12, 12, 1200, 0, army)  
    ...
    CreateInitialFireballSmokeRing = function(self)
        ...
        for i = 0, (sides-1) do
            ...
            self:CreateProjectile('/mods/Reinforcement Manager/effects/Entities/Seraphim/TacNukeShockwave01/TacNukeShockwave01_proj.bp', X * OffsetMod , 0.25, Z * OffsetMod, X, 0, Z)
    ...
    CreateOuterRingWaveSmokeRing = function(self)
        ...
        for i = 0, (sides-1) do
            local X = math.sin(i*angle)
            local Z = math.cos(i*angle)
            local proj =  self:CreateProjectile('/mods/Reinforcement Manager/effects/Entities/Seraphim/TacNukeShockwave02/TacNukeShockwave02_proj.bp', X * OffsetMod , 0.5, Z * OffsetMod, X, 0, Z)
    ...
    CreateHeadConvectionSpinners = function(self)
        ...
        for i = 0, (sides-1) do
            ...
            local proj = self:CreateProjectile('/mods/Reinforcement Manager/effects/Entities/Seraphim/TacNukeEffect03/TacNukeEffect03_proj.bp', x, HeightOffset, z, x, 0, z)
                :SetVelocity(velocity)
            table.insert(projectiles, proj)
        end   
    ...
    CreateGroundPlumeConvectionEffects = function(self,army)
		for k, v in ModEffectTemplate.SNukeGroundConvectionEffects01 do
			CreateEmitterAtEntity(self, army, v ):ScaleEmitter(0.25)
		end
		for k, v in ModEffectTemplate.SNukeGroundConvectionEffects02 do
			CreateEmitterAtEntity(self, army, v ):ScaleEmitter(0.25)
		end
		...
		for i = 0, (sides-1)
			...
			self:CreateProjectile('/mods/Reinforcement Manager/effects/Entities/Seraphim/TacNukeEffect05/TacNukeEffect05_proj.bp', x, RandomFloat(outer_lower_height, outer_upper_height), z, x, 0, z)
